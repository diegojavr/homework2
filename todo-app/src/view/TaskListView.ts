...
export default class HTMLTaskListView implements DOMList {
    ...
    private createTaskListElement(task: TaskItem): HTMLLIElement {
        ...
        li.className = "list-group-item d-flex justify-content-between gap-3 align-items-center";
        ...
        const spanForButtons = this.createSpanForButtons(saveButton, editButton, deleteButton);

        li.append(checkBox, editTaskInput, label, spanForButtons);
        return li;
    }
    private createCheckBox(task: TaskItem) {
    ...
    private createLabel(task: TaskItem) {
    ...
    private createEditAndSaveButton(editTaskInput: HTMLInputElement, label: HTMLLabelElement, task: TaskItem): HTMLButtonElement[] {
        ...
        saveButton.addEventListener("click", () => {
            ...
            this.taskListController.editTask(task.id, newTaskText);
            saveButton.hidden = true;
    ...
    private createSpanForButtons(saveButton: HTMLButtonElement, editButton: HTMLButtonElement, deleteButton: HTMLButtonElement) {
        const span = document.createElement("span") as HTMLSpanElement;
        span.className = "d-flex justify-content-end gap-3";
        span.append(saveButton, editButton, deleteButton);
        return span;
    }
